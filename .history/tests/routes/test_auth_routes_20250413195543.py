...
def test_register(client, monkeypatch):
    ...
    response = client.post("/api/auth/register", json=user_data)  # Змінено шлях
...
def test_repeat_register_username(client, monkeypatch):
    ...
    response = client.post("/api/auth/register", json=user_copy)  # Змінено шлях
...
def test_not_confirmed_login(client):
    response = client.post("/api/auth/login",  # Змінено шлях
...
def test_refresh_token(client):
    ...
    response = client.post("/api/auth/refresh", json={"refresh_token": refresh_token})  # Змінено шлях
    assert response.status_code == 200, response.text
    data = response.json()
    assert "access_token" in data
    assert "token_type" in data
...
def test_logout(client):
    with patch("src.services.auth.redis_client") as redis_mock:
        ...
        response = client.post("/api/auth/logout", json={"refresh_token": refresh_token},  # Змінено шлях
