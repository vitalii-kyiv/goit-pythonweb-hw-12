...
# Створення async_engine для тестів
@pytest_asyncio.fixture()
...
# Підготовка бази даних перед тестами
...
def user_data():
    return UserCreate(
        username=f"testuser{uuid4()}",  # Генерація унікального імені користувача
        email=f"testuser{uuid4()}@example.com",  # Генерація унікального email
        password="password123",
    )

# Фікстура для репозиторію користувачів
@pytest.fixture
...
async def test_create_user(user_repo, user_data):
    ...
    assert user.username == user_data.username  # Перевірка на унікальність імені
    assert user.email == user_data.email  # Перевірка на унікальність email
